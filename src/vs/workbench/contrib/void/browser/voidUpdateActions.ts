...
const notifyUpdate = (res: VoidCheckUpdateRespose & { message: string }, notifService: INotificationService, updateService: IUpdateService): INotificationHandle => {
	const message = res?.message || 'This is a very old version of NeoVoid, please download the latest version! [NeoVoid](https://github.com/graeme68s/neovoid/releases)!'
	...
	if (res?.action) {
		...
		if (res.action === 'reinstall') {
			primary.push({
				...
				run: () => {
					...
					window.open('https://github.com/graeme68s/neovoid/releases')
				}
			})
		}
		...
		primary.push({
			...
			run: () => {
				const { window } = dom.getActiveWindow()
				window.open('https://github.com/graeme68s/neovoid/')
			}
		})
...
const notifyErrChecking = (notifService: INotificationService): INotificationHandle => {
	const message = `NeoVoid Error: There was an error checking for updates. If this persists, please get in touch or reinstall NeoVoid [here](https://github.com/graeme68s/neovoid/releases)!`
	const notifController = notifService.notify({
		severity: Severity.Info,
		message: message,
		sticky: true,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('voidCheckUpdate', 'NeoVoid: Check for Updates'),
		});
	}
